...
class HrIqama(models.Model):
    ...
    def _get_age(self):
        ...
        if self.birthdate:
            try:
                self.age = (fields.Date.today() - self.birthdate).days / 365
            except:
                self.age = 0.0
        else:
            self.age = 0.0
    ...
    branch_id = fields.Many2one('res.branch', readonly=True, string='Office')
    ...
    def onchange_employee_id(self):
        ...
        self.profession = self.employee_id.profession
    ...
    def check_iqama_expiry(self):
        ...
        try:
            ...
            template_id = False
        next_date = fields.Date.today() + timedelta(days=30)
        for iqama in self.search([('state', '=', 'received'), ('expiry_date', '>=', fields.Date.today()),
                                  ('expiry_date', '<=', next_date)]):
            diff = iqama.expiry_date - fields.Date.today()
    ...
    def iqama_inprogress(self):
        ...
        for iqama in self:
            ...
            iqama.message_post(message_type="email", subtype='mail.mt_comment', body=_('IQAMA request is In progress'))
    ...
    def iqama_refuse(self):
        ...
        """
        for iqama in self:
            iqama.write({'state': 'refuse',
                         'refused_by': self.env.uid,
                         'refused_date': datetime.today()})
            iqama.message_post(message_type="email", subtype='mail.mt_comment', body=_('IQAMA request is Refused'))
    ...
    def iqama_received(self):
        ...
        """
        for iqama in self:
            if iqama.handled_by_id.user_id:
                iqama.message_subscribe(partner_ids=[self.handled_by_id.user_id.partner_id.id])
            iqama.state = 'received'
            iqama.message_post(message_type="email", subtype='mail.mt_comment', body=_('IQAMA request is Received'))
    ...
    def iqama_validate(self):
        ...
        """
        for iqama in self:
            iqama.write({'state': 'validate',
                         'approved_by': self.env.uid,
                         'approved_date': datetime.today()})
            iqama.message_post(message_type="email", subtype='mail.mt_comment', body=_('IQAMA request is Validated'))
