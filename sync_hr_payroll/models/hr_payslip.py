...
class HrPayslip(models.Model):
    ...
    work_email_data = fields.Text(readonly=True)
    draft_payslip_data = fields.Text(readonly=True)

    def check_duplicate_record(self):
        for rec in self:
            get_param = self.env['ir.config_parameter'].sudo().get_param('base_setup.duplicate_payslip_restrict')
            if get_param:
                clause_1 = ['&', ('date_to', '<=', rec.date_to), ('date_to', '>=', rec.date_from)]
                clause_2 = ['&', ('date_from', '<=', rec.date_to), ('date_from', '>=', rec.date_from)]
                clause_3 = ['&', ('date_from', '<=', rec.date_from), '|', ('date_to', '=', False),
                            ('date_to', '>=', rec.date_to)]
                clause_final = [('employee_id', '=', rec.employee_id.id), ('state', '=', 'done'), ('id', '!=', rec.id), '|',
                ...
                if len(rec_ids) % 2 == 0 and rec.credit_note:
                    ...
                    raise ValidationError(_(
                        'You already generated payslip with same duration of "%s".Kindly check Once.' % (
                                rec.name)))

    def send_payslip(self):
        if self.employee_id.work_email:
            template = self.env.ref('hr_payroll.send_payslip_mail', raise_if_not_found=False)
            template.send_mail(self.id)
            self.is_send = True
        else:
            raise UserError(_("Please Set %s's %s Email and Confirm Payslip:") % self.employee_id.name,
                            self.eployee_id.last_name)
