...
from odoo.addons.sync_hr_payroll.tests.common import TestPayslipBase
...
class TestPayslipFlow(TestPayslipBase):
    ...
    def test_00_payslip_flow(self):
        ...
        context = {
            ...
            "active_ids": [self.ref("sync_hr_payroll.menu_department_tree")],
            "active_id": self.ref("sync_hr_payroll.menu_department_tree")
        }
        ...
        data, data_format = self.env.ref('sync_hr_payroll.action_report_payslip').render(richard_payslip.ids)

        # I print the payslip details report
        data, data_format = self.env.ref('sync_hr_payroll.payslip_details_report').render(richard_payslip.ids)

        # I print the contribution register report
        context = {'model': 'hr.contribution.register', 'active_ids': [self.ref('sync_hr_payroll.hr_houserent_register')]}
