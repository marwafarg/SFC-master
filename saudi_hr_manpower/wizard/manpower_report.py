...
class ManpowerReport(models.TransientModel):
    ...
    def print_manpower_report(self):
        ...
        manpower_dict = {}
        for plan_line in manpower_obj.search([('plan_id.fiscal_year_id', '=', report.fiscal_year_id.id)]):
            manpower_dict.update({plan_line.id: manpower_dict.get(plan_line.id, {})})
            joining_dict = {}
            line_dict = {
                         'title': plan_line.job_id.name or '',
                         'actual_emp': plan_line.current_employees or 0, 'forecasted_leavers': plan_line.leaving_employees,
                         'calculated_emp': plan_line.calculated_employees, 'critical_roles': plan_line.critical_roles,
                         'expected_emp': plan_line.expected_employees, 'future_strength': plan_line.future_strength}
        ...
        worksheet.merge_range(0, 3, 0, 7, 'Manpowerplan Report for :%s' % (report.fiscal_year_id.name), detail_head)
        worksheet.write(2, 0, 'Title', detail_head)
        worksheet.write(2, 2, 'Actual \n Emp', detail_head)
        worksheet.write(2, 3, 'Forecasted \n Leavers', detail_head)
        worksheet.write(2, 4, 'Actual Emp - \n Forecasted Leavers \n E(C-D)', detail_head)
        worksheet.write(2, 5, 'Critical \n Roles', detail_head)
        worksheet.write(2, 6, 'Expected \n New \n Hires', detail_head)
        first_row = 1
        from_col = to_col = 7
        all_col_dict = {'title': 0,
                        'actual_emp': 2, 'forecasted_leavers': 3,
                        'calculated_emp': 4, 'critical_roles': 5,
                        'expected_emp': 6}
            ...
            for plan_key in plan_dict.keys():
                if plan_key != 'joining_months':
                    ...
                    for month in plan_dict[plan_key]:
                        worksheet.write(actual_data_row, all_col_dict.get(month), month_dict[month], details_format)
        worksheet.set_row(actual_data_row + 1, 30, detail_head)
        worksheet.write(actual_data_row + 1, all_col_dict.get('title'), 'Total', detail_head)
        for col in range(2, all_col_dict.get('future_strength') + 1):
            ...
            worksheet.write_formula(actual_data_row + 1, col, '{=SUM(%s:%s)}' % (service_first, service_last), detail_footer)
        workbook.close()
        export_id = self.env['manpower.report'].create({'fiscal_year_id': report.fiscal_year_id.id, 'excel_file': base64.encodestring(fp.getvalue()), 'filename': filename})
        fp.close()
