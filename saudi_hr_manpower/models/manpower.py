...
class ManpowerPlan(models.Model):
    ...
    fiscal_year_id = fields.Many2one('year.year', 'Plan Year', required=True,
    ...
    state = fields.Selection([('draft', 'Draft'),
                              ...
                              ('approved', 'Approved'),
                              ('rejected', 'Rejected')], 'Status', readonly=True, default='draft')

    def name_get(self):
        """
            to use retrieving the name, combination of `department name & fiscal year name`
        """
        result = []
        for plan in self:
            name = ''.join([plan.department_id.name or '', ' - ', plan.fiscal_year_id.name or ''])
    ...
    def onchange_department(self):
        """
            onchange the value based on selected hof, department
        """
        self.hof_id = False
    ...
    def action_confirm_manpower(self):
        ...
        self.ensure_one()
        self.state = 'confirm'
        self.message_post(message_type="email", subtype='mail.mt_comment', body=_('Manpower plan confirm'))
    ...
    def action_approve_manpower(self):
        ...
        self.ensure_one()
        self.state = 'approved'
        self.message_post(message_type="email", subtype='mail.mt_comment', body=_('Manpower plan approve.'))
    ...
    def action_reject_manpower(self):
        ...
        self.ensure_one()
        self.state = 'rejected'
        self.message_post(message_type="email", subtype='mail.mt_comment', body=_('Manpower plan reject'))
    ...
    def action_set_draft(self):
        ...
        self.ensure_one()
        self.state = 'draft'
        self.message_post(message_type="email", subtype='mail.mt_comment', body=_('Manpower plan draft'))
...
class ManpowerPlanLine(models.Model):
    ...
    def _calculate_employees(self):
        ...
        for line in self:
            ...
            line.future_strength = calculated_emp + line.critical_roles + line.expected_employees
    ...
    calculated_employees = fields.Integer('Forecast Employees', compute=_calculate_employees)
    critical_roles = fields.Integer('Critical Roles')
    expected_employees = fields.Integer('Expected New Hires', compute=_calculate_expected_emp)
    future_strength = fields.Integer('Future Workforce', compute=_calculate_employees)
    joining_months = fields.One2many('manpower.joining', 'plan_line_id', 'Joining Details')
        ...
        for rec in self:
            ...
            for month in self.joining_months:
                if month.period_id.id not in month_list:
                    ...
                    raise ValidationError(_('You already set %s month plan for %s job, Kindly check!!') %
    ...
    def onchange_job_id(self):
        ...
        if self.job_id:
            ...
            for emp in employee_ids:
                current_emp += 1
            self.current_employees = current_emp
    ...
    def create_or_update_requisition(self):
        ...
        if self.joining_months:
            ...
            if not self.job_requisition_id:
                job_req_id = self.env['hr.job.requisition'].create({'name': self.job_id.name,
    ...
    def _progress_bar(self):
        ...
        for rec in self:
            ...
            rec.progress = (total_emp * 100) / rec.expected_employees if rec.expected_employees else 0
...
class ManpowerJoining(models.Model):
    ...
    current_recruitment_emp = fields.Integer('Current Recruitment', compute='_calculate_cur_recruitment')
    progress = fields.Float(string='Progress', compute='_progress_bar')
    ...
    def _calculate_cur_recruitment(self):
        ...
        for rec in self:
            no_of_curr_rec = 0
            employee_ids = self.env['hr.employee'].search([('date_of_join', '>=', rec.period_id.date_start),
                                                           ('date_of_join', '<=', rec.period_id.date_stop),
                                                           ('job_id', '=', rec.plan_line_id.job_id.id)])
            for emp in employee_ids:
                no_of_curr_rec += 1
            rec.current_recruitment_emp = no_of_curr_rec
    ...
    def _progress_bar(self):
        ...
        for rec in self:
            if rec.current_recruitment_emp > 0 and rec.joining_employees > 0:
                rec.progress = (rec.current_recruitment_emp * 100)/rec.joining_employees
            else:
                rec.progress = 0.0
