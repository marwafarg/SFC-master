...
class HrTransactionReport(models.TransientModel):
    ...
    employee_selection = fields.Selection([
            ('all', 'All'),
            ('selected', 'Selected')
        ], string="Employee Selection", required=True, default='all')
    ...
    def get_employee_vacation_details(self, employees, vacation_details):
        ...
        vacation_ids = self.env['hr.vacation'].search([('employee_id', 'in', employees.ids),
        ...
        for vacation in vacation_ids:
            vacation_details.append({
                ...
                'department': vacation.employee_id.department_id.name if vacation.employee_id.department_id else '',
                'vacation_type': 'unpaid',
                'from_date': vacation.date_start,
                'to_date': vacation.date_to,
                'return_date': vacation.return_to_work_date if vacation.return_to_work_date else ''
            })

    def get_employee_transfer_details(self, employees, transfer_details):
        transfer_employee_ids = self.env['transfer.employee'].search([('employee_id', 'in', employees.ids), 
        ...
        for transfer in transfer_employee_ids:
            transfer_details.append({
                ...
                'new_branch': transfer.new_branch_id.name if transfer.new_branch_id else '',
                'transfer_date': transfer.effective_date.strftime(DF) if transfer.effective_date else '',
            })

    def get_employee_clearance_details(self, employees, clearance_details):
        date_from = datetime.strptime(self.start_date.strftime(DT), DT)
        date_to = datetime.strptime(self.end_date.strftime(DT), DT)
        clearance_employee_ids = self.env['hr.employee.clearance'].search([('employee_id', 'in', employees.ids), 
        ...
        for clearance in clearance_employee_ids:
            clearance_details.append({
                ...
                'department': clearance.department_id.name if clearance.department_id else '',
                'reason': '',
                'last_day': clearance.last_working_day.strftime(DF) if clearance.last_working_day else '',
            })

    def get_employee_other_hr_payslip_details(self, employees, other_hr_payslip_details):
        other_hr_payslip_ids = self.env['other.hr.payslip'].search([('employee_id', 'in', employees.ids),
        ...
        for other_hr_payslip in other_hr_payslip_ids:
            other_hr_payslip_details.append({
                ...
                'department': other_hr_payslip.department_id.name if other_hr_payslip.department_id else '',
                'operation_type': dict(other_hr_payslip._fields['operation_type'].selection).get(other_hr_payslip.operation_type),
                'description': other_hr_payslip.description
            })

    def get_new_joining_emp_details(self, new_joining_emp_details):
        employees_ids = self.env['hr.employee'].search([('date_of_join', '>=', self.start_date),
        ...
        for emp in employees_ids:
            new_joining_emp_details.append({
                ...
                'hiring_date': emp.date_of_join.strftime(DF) if emp.date_of_join else '',
                # 'status': dict(emp._fields['employee_status'].selection).get(emp.employee_status) if emp.employee_status else '',
            })

    def get_employee_leaves_details(self, employees, leaves_details):
        date_from = datetime.strptime(self.start_date.strftime(DT), DT)
        date_to = datetime.strptime(self.end_date.strftime(DT), DT)
        leave_ids = self.env['hr.leave'].search([('employee_id', 'in', employees.ids), 
                                                 ('date_from', '>=', date_from),
                                                 ('date_to', '<=', date_to),
                                                 ('state', '=', 'validate')],
        ...
        for leave in leave_ids:
            leaves_details.append({
                ...
                'department': leave.employee_id.department_id.name if leave.employee_id.department_id else '',
                'leave_type': leave.holiday_status_id.name,
                'from_date': datetime.strftime(leave.date_from, DT),
                'to_date': datetime.strftime(leave.date_to, DT),
                # 'state': dict(leave._fields['state'].selection).get(leave.state)
            })

    def get_employee_loan_details(self, employees, loan_details):
        loan_ids = self.env['hr.loan'].search([('employee_id', 'in', employees.ids)],
                                                order="state, employee_id, department_id")
        for loan in loan_ids:
            deduction = 0.0
            installment_lines = loan.installment_lines.filtered(lambda i:i.date >= self.start_date and i.date <= self.end_date)
            ...
            loan_details.append({
                ...
                'location': loan.employee_id.work_location,
                'date': loan.request_date,
                'loan_amount': loan.loan_amount,
                'monthly_payment': loan.loan_amount / loan.duration if loan.duration != 0 else '',
    ...
    def get_employee_warning_details(self, employees, warning_details):
        date_from = datetime.strptime(self.start_date.strftime(DT), DT)
        date_to = datetime.strptime(self.end_date.strftime(DT), DT)
        warning_ids = self.env['issue.warning'].search([('warning_date', '>=', date_from),
        ...
        for warning in warning_ids:
            ...
            for emp in employee_ids:
                warning_details.append({
                    'employee_code': emp.code,
                    'employee_name': emp.name,
                    'warning_action': dict(warning._fields['warning_action'].selection).get(warning.warning_action),
    ...
    def get_employee_expense_details(self, employees, expense_details, total_expense):
        date_from = datetime.strptime(self.start_date.strftime(DT), DT)
        date_to = datetime.strptime(self.end_date.strftime(DT), DT)
        expense_ids = self.env['hr.expense'].search([('employee_id', 'in', employees.ids),
                                                     ('date', '>=', date_from),
                                                     ('date', '<=', date_to),
                                                     ('state', '=', 'done')],
        ...
        for expense in expense_ids:
            expense_details.append({
                ...
                'date': datetime.strftime(expense.date, DT),
                'reason': expense.product_id.name,
                'description': expense.name,
                'reference': expense.reference,
            })
        for employee in employees:
            expense_amount = sum(expense_ids.filtered(lambda e: e.employee_id.id == employee.id).mapped('unit_amount'))
            total_expense.append({employee.id: expense_amount})

    def get_employee_payslip_details(self, employees, payslip_details, payslip_headers, total_expense, cost_details):
        date_from = datetime.strptime(self.start_date.strftime(DT), DT)
        date_to = datetime.strptime(self.end_date.strftime(DT), DT)
        payslip_ids = self.env['hr.payslip'].search([('employee_id', 'in', employees.ids),
                                                     ('date_from', '>=', date_from),
                                                     ('date_to', '<=', date_to),
                                                     ('state', '=', 'done')],
        ...
        for employee in employees:
            employee_payslips = payslip_ids.filtered(lambda e: e.employee_id.id == employee.id)
            net_salary = 0.0
            for emp_payslip in employee_payslips:
                payslip_line = emp_payslip.line_ids.filtered(lambda x: x.code == 'NET' and x.salary_rule_id in rule_codes)
                if payslip_line:
            ...
            # expense_amount = sum([expense.get(employee.id) for expense in total_expense])
            cost_details.append({
    ...
    def generate_hr_transaction_excel_report(self):
        ...
        f_name = 'hr_transaction_report.xlsx'
        fp = BytesIO()
        ...
        # date_time_format = workbook.add_format({'align': 'left', 'num_format': '#,##0.00', 'border': 1, 'size': 10})
        date_time_format = workbook.add_format({'align': 'left', 'num_format': 'mm/dd/yy', 'border': 1, 'size': 10})
        leaves_details, transfer_details, clearance_details, other_hr_payslip_details,\
            new_joining_emp_details, loan_details, vacation_details, warning_details,\
            expense_details, payslip_details, payslip_headers, total_expense,\
            cost_details = [], [], [], [], [], [], [], [], [], [], [], [], []

        if not self.add_vacation and not self.add_transfer and not self.add_clearance and not self.add_other_allowance\
            and not self.add_new_staff and not self.add_sick_leave and not self.add_loan\
            and not self.add_warning and not self.add_cost_report:
            raise ValidationError(_("Please select at least one checkbox for print report."))
        ...
        if self.add_other_allowance:
            self.get_employee_other_hr_payslip_details(employee_ids, other_hr_payslip_details)
        ...
        if self.add_cost_report:
            self.get_employee_expense_details(employee_ids, expense_details, total_expense)
            self.get_employee_payslip_details(employee_ids, payslip_details, payslip_headers, total_expense, cost_details)
        ...
        try:
            ...
            worksheet.write(row+1, column+4, 'Print By', cell_heading_format)

            worksheet.write(row, column+2, self.start_date.strftime(DF), heading_data_format)
            worksheet.write(row, column+5, self.end_date.strftime(DF), heading_data_format)
            worksheet.write(row+1, column+2, fields.Date.today().strftime(DF), heading_data_format)
            worksheet.write(row+1, column+5, self.env.user.name, heading_data_format)
            row += 3

            if self.add_vacation:
                # Vacation summary details
                worksheet.merge_range('A%s:H%s' %(row+2, row+3), 'Vacation Summary', sub_heading_format)
                row += 3
                ...
                for num, vacation in enumerate(vacation_details, 1):
                    ...
                    worksheet.write(row, column+1, vacation['employee_code'], cell_data_format)
                    worksheet.write(row, column+2, vacation['employee_name'], cell_data_format)
                    worksheet.write(row, column+3, vacation['department'], cell_data_format)
                    worksheet.write(row, column+4, vacation['vacation_type'], cell_data_format)
                    worksheet.write(row, column+5, vacation['from_date'], date_time_format)
                    worksheet.write(row, column+6, vacation['to_date'], date_time_format)
                    worksheet.write(row, column+7, vacation['return_date'], date_time_format)
                    row += 1

            if self.add_transfer:
                row += 1
                # Transfer details
                worksheet.merge_range('A%s:F%s' %(row+2, row+3), 'Transfer', sub_heading_format)
                row += 3
                worksheet.write(row, column, 'SN', cell_heading_format)
                worksheet.write(row, column+1, 'Employee Code', cell_heading_format)
                worksheet.write(row, column+2, 'Employee Name', cell_heading_format)
                ...
                for num_transfer, transfer in enumerate(transfer_details, 1):
                    ...
                    worksheet.write(row, column+1, transfer['employee_code'], cell_data_format)
                    worksheet.write(row, column+2, transfer['employee_name'], cell_data_format)
                    worksheet.write(row, column+3, transfer['old_branch'], cell_data_format)
                    worksheet.write(row, column+4, transfer['new_branch'], cell_data_format)
                    worksheet.write(row, column+5, transfer['transfer_date'], date_time_format)
                    row += 1

            if self.add_clearance:
                row += 1
                # Clearance details
                worksheet.merge_range('A%s:F%s' %(row+2, row+3), 'Clearance', sub_heading_format)
                row += 3
                worksheet.write(row, column, 'SN', cell_heading_format)
                worksheet.write(row, column+1, 'Employee Code', cell_heading_format)
                worksheet.write(row, column+2, 'Employee Name', cell_heading_format)
                ...
                worksheet.write(row, column+5, 'Last Day', cell_heading_format)
                row += 1
                for num_clearance, clearance in enumerate(clearance_details, 1):
                    ...
                    worksheet.write(row, column+1, clearance['employee_code'], cell_data_format)
                    worksheet.write(row, column+2, clearance['employee_name'], cell_data_format)
                    worksheet.write(row, column+3, clearance['department'], cell_data_format)
                    worksheet.write(row, column+4, clearance['reason'], cell_data_format)
                    worksheet.write(row, column+5, clearance['last_day'], date_time_format)
                    row += 1

            if self.add_other_allowance:
                row += 1
                # Other Allowances/Deduction
                worksheet.merge_range('A%s:F%s' %(row+2, row+3), 'Other Allowances/Deduction', sub_heading_format)
                row += 3
                worksheet.write(row, column, 'SN', cell_heading_format)
                worksheet.write(row, column+1, 'Employee Code', cell_heading_format)
                worksheet.write(row, column+2, 'Employee Name', cell_heading_format)
                worksheet.write(row, column+3, 'Department', cell_heading_format)
                worksheet.write(row, column+4, 'Deduct /Allowances', cell_heading_format)
                worksheet.write(row, column+5, 'Reason', cell_heading_format)
                row += 1
                for num_other, other_hr_payslip in enumerate(other_hr_payslip_details, 1):
                    worksheet.write(row, column, str(num_other), cell_data_format)
                    worksheet.write(row, column+1, other_hr_payslip['employee_code'], cell_data_format)
                    worksheet.write(row, column+2, other_hr_payslip['employee_name'], cell_data_format)
                    worksheet.write(row, column+3, other_hr_payslip['department'], cell_data_format)
                    worksheet.write(row, column+4, other_hr_payslip['operation_type'], cell_data_format)
                    worksheet.write(row, column+5, other_hr_payslip['description'], cell_data_format)
                    row += 1

            if self.add_new_staff:
                row += 1
                # New Staff
                worksheet.merge_range('A%s:G%s' %(row+2, row+3), 'New Staff', sub_heading_format)
                row += 3
                worksheet.write(row, column, 'SN', cell_heading_format)
                worksheet.write(row, column+1, 'Employee Code', cell_heading_format)
                worksheet.write(row, column+2, 'Employee Name', cell_heading_format)
                ...
                # worksheet.write(row, column+6, 'Status', cell_heading_format)
                row += 1
                for emp_num, emp in enumerate(new_joining_emp_details, 1):
                    worksheet.write(row, column, str(emp_num), cell_data_format)
                    worksheet.write(row, column+1, emp['employee_code'], cell_data_format)
                    worksheet.write(row, column+2, emp['employee_name'], cell_data_format)
                    worksheet.write(row, column+3, emp['job_title'], cell_data_format)
                    worksheet.write(row, column+4, emp['department'], cell_data_format)
                    worksheet.write(row, column+5, emp['hiring_date'], date_time_format)
            ...
            if self.add_sick_leave:
                ...
                worksheet.merge_range('A%s:H%s' %(row+2, row+3), 'Leave Summary', sub_heading_format)
                row += 3
                worksheet.write(row, column, 'SN', cell_heading_format)
                worksheet.write(row, column+1, 'Employee Code', cell_heading_format)
                worksheet.write(row, column+2, 'Employee Name', cell_heading_format)
                ...
                for num, leave in enumerate(leaves_details, 1):
                    worksheet.write(row, column, str(num), cell_data_format)
                    worksheet.write(row, column+1, leave['employee_code'], cell_data_format)
                    worksheet.write(row, column+2, leave['employee_name'], cell_data_format)
                    worksheet.write(row, column+3, leave['department'], cell_data_format)
                    worksheet.write(row, column+4, leave['leave_type'], cell_data_format)
                    worksheet.write(row, column+5, leave['from_date'], date_time_format)
            ...
            if self.add_loan:
                row += 1
                # Loan details
                worksheet.merge_range('A%s:L%s' %(row+2, row+3), 'Loan Summary', sub_heading_format)
                row += 3
                worksheet.write(row, column, 'SN', cell_heading_format)
                worksheet.write(row, column+1, 'Employee Code', cell_heading_format)
                worksheet.write(row, column+2, 'Employee Name', cell_heading_format)
                ...
                for loan_num, loan in enumerate(loan_details, 1):
                    ...
                    worksheet.write(row, column+1, loan['employee_code'], cell_data_format)
                    worksheet.write(row, column+2, loan['employee_name'], cell_data_format)
                    ...
                    worksheet.write(row, column+6, loan['loan_amount'], cell_data_format)
                    worksheet.write(row, column+7, loan['monthly_payment'], cell_data_format)
                    worksheet.write(row, column+8, loan['total_deduction'], cell_data_format)
                    worksheet.write(row, column+9, loan['deduction'], cell_data_format)
                    worksheet.write(row, column+10, loan['balance'], cell_data_format)
            ...
            if self.add_warning:
                row += 1
                # Warning details
                worksheet.merge_range('A%s:G%s' %(row+2, row+3), 'Warning', sub_heading_format)
                row += 3
                worksheet.write(row, column, 'SN', cell_heading_format)
                worksheet.write(row, column+1, 'Employee Code', cell_heading_format)
                worksheet.write(row, column+2, 'Employee Name', cell_heading_format)
                ...
                for warning_num, warning in enumerate(warning_details, 1):
                    ...
                    worksheet.write(row, column+1, warning['employee_code'], cell_data_format)
                    worksheet.write(row, column+2, warning['employee_name'], cell_data_format)
                    worksheet.write(row, column+3, warning['warning_action'], cell_data_format)
                    worksheet.write(row, column+4, warning['warning_date'], date_time_format)
                    worksheet.write(row, column+5, warning['warning_type'], cell_data_format)
                    worksheet.write(row, column+6, warning['description'], cell_data_format)
                    row += 1

            if self.add_cost_report:
                row += 1
                # Expense details
                worksheet.merge_range('A%s:H%s' %(row+2, row+3), 'Expense Report', sub_heading_format)
                row += 3
                worksheet.write(row, column, 'SN', cell_heading_format)
                worksheet.write(row, column+1, 'Employee Code', cell_heading_format)
                worksheet.write(row, column+2, 'Employee Name', cell_heading_format)
                ...
                for expense_num, expense in enumerate(expense_details, 1):
                    ...
                    worksheet.write(row, column+1, expense['employee_code'], cell_data_format)
                    worksheet.write(row, column+2, expense['employee_name'], cell_data_format)
                    worksheet.write(row, column+3, expense['expense_amount'], cell_data_format)
                    worksheet.write(row, column+4, expense['date'], date_time_format)
                    worksheet.write(row, column+5, expense['reason'], cell_data_format)
                    worksheet.write(row, column+6, expense['description'], cell_data_format)
                    worksheet.write(row, column+7, expense['reference'], cell_data_format)
                    row += 1

                row += 1
                # Payslip details
                worksheet.merge_range('A%s:H%s' %(row+2, row+3), 'Payslip Details', sub_heading_format)
                row += 3
                worksheet.write(row, column, 'SN', cell_heading_format)
                col = column+1
                ...
                row += 1
                # Cost details
                worksheet.merge_range('A%s:F%s' %(row+2, row+3), 'Cost Report', sub_heading_format)
                row += 3
                worksheet.write(row, column, 'SN', cell_heading_format)
                worksheet.write(row, column+1, 'Employee Code', cell_heading_format)
                worksheet.write(row, column+2, 'Employee Name', cell_heading_format)
                ...
                for num_cost, cost in enumerate(cost_details, 1):
                    ...
                    worksheet.write(row, column+1, cost['employee_code'], cell_data_format)
                    worksheet.write(row, column+2, cost['employee_name'], cell_data_format)
                    worksheet.write(row, column+3, cost['expense_amount'], cell_data_format)
                    worksheet.write(row, column+4, cost['net_salary'], cell_data_format)
                    worksheet.write(row, column+5, cost['total_cost'], cell_data_format)
                    row += 1
        ...
        self.write({'filename': file_name, 'excel_file': base64.encodestring(fp.getvalue())})
        fp.close()
        return {
