...
class LeaveEncashment(models.TransientModel):
    ...
    encashment_type = fields.Selection([('by_payslip', 'By Payslip'), ('reduse_days', 'Reduse Days'), ('carry_forward', 'Carry Forward Next Year'), ('bank_cash', 'Bank/Cash')], default='by_payslip', required=True)
        ...
        for rec in self:
            ...
            if sum(rec.leave_encashment_details_ids.mapped('encashment_days')) > rec.remaining_leaves:
                ...
                if rec.encashment_type == 'carry_forward':
                    ...
                    if new_allocation_id and new_allocation_id.holiday_status_id.validation_type == 'both':
...
class LeaveEncashmentDetails(models.TransientModel):
    ...
    def _get_allocation_domain(self):
        ...
        for allocation in allocation_ids:
            if allocation.holiday_status_id.validity_start.year == int(year):
