...
class BusinessCard(models.TransientModel):
    ...
    def print_business_card(self):
        ...
        row += 1
        worksheet.merge_range(row, col, row, col + 2, emp_card.branch_id.country_id or '', details_format)
        row += 2
        ...
        row += 2
        workbook.close()
        export_id = self.env['business.card'].create({'excel_file': base64.encodestring(fp.getvalue()),
                                                      'filename': filename})
        fp.close()
