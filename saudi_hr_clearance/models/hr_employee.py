...
class HREmployee(models.Model):
    ...
    def _compute_clearance_count(self):
        self.clearance_count = self.env['hr.employee.clearance'].search_count([('employee_id', '=', self.id)])

    def action_view_clearance(self):
        self.ensure_one()
        clearance_ids = self.env['hr.employee.clearance'].search([('employee_id', '=', self.id)])
