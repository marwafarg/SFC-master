...
class HrEmployeeEos(models.Model):
    ...
    def _compute_clearance_mandatory_for_eos(self):
        clearance_mandatory_for_eos = self.env["ir.config_parameter"].sudo().get_param("saudi_hr_eos.clearance_mandatory_for_eos")
        for record in self:
            record.clearance_mandatory_for_eos = clearance_mandatory_for_eos

    clearance_mandatory_for_eos = fields.Boolean('Clearance Mandatory for EOS', copy=False, compute='_compute_clearance_mandatory_for_eos',
                        default=lambda self: self.env["ir.config_parameter"].sudo().get_param("saudi_hr_eos.clearance_mandatory_for_eos"))
    ...
    def check_clearance(self):
        ...
        if self.clearance_mandatory_for_eos:
            ...
            if not clearance_id:
                raise UserError(_("There is a no clearance for this employee please create clearance first."))
    ...
    def create_clearance(self):
        ...
        self.ensure_one()
        ClearanceObj = self.env['hr.employee.clearance']
        clearance_id = ClearanceObj.create({
            'employee_id': self.employee_id.id,
