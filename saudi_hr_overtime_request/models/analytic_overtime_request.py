...
class AnalyticOvertime(models.Model):
    ...
    user_id = fields.Many2one('res.users', 'Responsible', track_visibility='onchange', default=lambda self: self.env.uid, readonly=True)
    ...
    branch_ids = fields.Many2many('res.branch', string='Offices')
    ...
    #     """
    #         set the value of employee_ids based on selected branch_ids
    #     """
    ...
    #     """
    #         set the value of employee_ids based on selected company_ids
    #     """
    #     employees = self.env['hr.employee'].search([('company_id', 'in', self.company_ids.ids)])
    #     self.attendee_ids = [(6, 0, employees.ids)]
    #
...
class AnalyticOvertimeAttendee(models.Model):
    ...
    state = fields.Selection([('needs-action', 'Waiting Answer'),
                              ('declined', 'Declined'),
                              ('accepted', 'Accepted')], 'Status', readonly=True, help="Status of the attendee's participation", default='needs-action', track_visibility='onchange')
