...
class HrEmployeeRecVisa(models.Model):
    ...
    state = fields.Selection([('draft', 'To Submit'), ('confirm', 'Confirm'), ('inprogress', 'In Progress'),
                              ('received', 'Received'), ('refused', 'Refused')], 'State', default='draft', track_visibility='onchange')
    ...
    def create(self, values):
        ...
        if values.get('employee_id'):
            ...
            values.update({'department_id': employee.department_id.id,
                           'nationality': employee.sudo().country_id.id,
                           'email': employee.work_email
                           })
        return super(HrEmployeeRecVisa, self).create(values)
