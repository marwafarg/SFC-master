...
class HrQualification(models.Model):
    ...
    def name_get(self):
        ...
        for rec in self:
            if rec.employee_id:
                name = ''.join([rec.employee_id.name, ' - ', rec.degree_id.name, ' - Qualification'])
            else:
...
class HrCertification(models.Model):
    ...
    def check_certificate_expiry(self):
        try:
            template_id = self.env.ref('saudi_hr_exp_info.cerificate_expiry_notification')
        except ValueError:
            template_id = False
...
class HrExperience(models.Model):
    ...
    def name_get(self):
        ...
        for rec in self:
            if rec.employee_id:
                name = ''.join([rec.employee_id.name, ' - ', rec.company, ' - Experience'])
            else:
    ...
    def action_send_mail(self):
        ...
        """
        template_id = self.env.ref('saudi_hr_exp_info.email_template_employment_reference', False)
