...
class InsurancePremiumMultiInvoice(models.TransientModel):
    ...
    def create_invoice_button(self):
        if self.start_date >= self.end_date:
            raise UserError(_("Error! premium start-date must be lower than premium end-date."))
        context = dict(self._context or {})
        invoice_obj = self.env['account.move']
        invoice_line_obj = self.env['account.move.line']
        active_ids = context.get('active_ids', []) or []
        product_id = self.env.ref('saudi_hr_medical.insurance_prodcuct')
        insurance_premium = self.env['insurance.premium'].browse(active_ids).filtered(lambda premium: premium.date >= self.start_date and premium.date <= self.end_date and premium.is_invoice_created == False)
        invoice_dict = {}
        for premium in insurance_premium:
            if premium.supplier_id.id not in invoice_dict:
                contract = self.env['hr.contract'].search([('employee_id', '=', premium.insurance_id.employee_id.id),
                                                            ('state', '=', 'open')], limit=1)
                invoice_dict[premium.supplier_id.id] = {'supplier': premium.supplier_id.id,
                                                        'invoice_origin': premium.insurance_id.name + '-' +str(premium.sequence),
                                                        'account_id': premium.insurance_id.supplier_id.property_account_payable_id.id,
                                                        'invoice_line': {'price_unit': premium.amount,
                                                                         },
                                                        'premium_ids': [premium.id],
                                                        'branch_id': premium.insurance_id.branch_id.id,
                                                        'analytic_account_id': contract and contract.analytic_account_id.id or False,
                                                        'analytic_tag_ids': contract and [(6, 0, contract.analytic_tag_ids.ids)] or False,}
                #invoice_dict[premium.supplier_id.id]['invoice_line']['price_unit']
            else:
                invoice_dict[premium.supplier_id.id]['invoice_line']['price_unit'] += premium.amount
                invoice_dict[premium.supplier_id.id]['premium_ids'].append(premium.id)
        for rec in invoice_dict.values():
            invoices_id = invoice_obj.create({'partner_id': rec['supplier'],
                                              'type': 'in_invoice',
            ...
            inv_line_val.update({'name': product_id.name,
                                 ...
                                 'account_id': (product_id.property_account_expense_id and product_id.property_account_expense_id.id or False) or (
            ...
            invoices_line_id = invoice_line_obj.with_context({'check_move_validity': False}).create(inv_line_val)
            ...
            premium_ids.write({'invoice_id': invoices_id.id, 'is_invoice_created': True})
