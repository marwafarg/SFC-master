...
class AdminReports(models.TransientModel):
    ...
    #
    # report = fields.Selection(selection_add=[('accommodation', 'Accommodation')])
    # accommodation_type = fields.Selection([('villa', 'Villa'), ('hotel', 'Hotel')], 'Stay In',)
    #
    # def print_accommodation_report(self):
    #     accommodation_line_obj = self.env['accommodation.line']
    #     report = self
    #     filename = 'Accommodation Report.xlsx'
    #     fp = BytesIO()
    ...
    #     if report.accommodation_type:
    #         domain.append(('accommodation_type', '=', report.accommodation_type))
    ...
    #         line_dict = {'employee': accommodation_line.accommodation_id.employee_id.name or '', 'type': accommodation_line.accommodation_type,
    #                      'villa': accommodation_line.villa_id.name or '-', 'hotel': accommodation_line.hotel_id.name or '-',
    #                      'city': accommodation_line.accommodation_id.city or '', 'check-in': datetime.strftime(accommodation_line.check_in_time, DEFAULT_SERVER_DATE_FORMAT),
    #                      'check-out': datetime.strftime(accommodation_line.check_out_time, DEFAULT_SERVER_DATE_FORMAT), 'days': accommodation_line.days,
    ...
    #     worksheet.write(1, 4, datetime.strftime(datetime.now(), DEFAULT_SERVER_DATE_FORMAT), details_format)
