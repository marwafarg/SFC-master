...
class Accommodation(models.Model):
    ...
    def _track_subtype(self, init_values):
        ...
        if 'state' in init_values and self.state == 'draft':
            return self.env.ref('saudi_hr_accommodation_book.mt_accommodation_new')
        elif 'state' in init_values and self.state == 'confirm':
            return self.env.ref('saudi_hr_accommodation_book.mt_accommodation_confirm')
        elif 'state' in init_values and self.state == 'validate':
            return self.env.ref('saudi_hr_accommodation_book.mt_accommodation_validate')
        elif 'state' in init_values and self.state == 'booked':
            return self.env.ref('saudi_hr_accommodation_book.mt_accommodation_book')
        elif 'state' in init_values and self.state == 'stay_over':
            return self.env.ref('saudi_hr_accommodation_book.mt_accommodation_stay_over')
        elif 'state' in init_values and self.state == 'refuse':
            return self.env.ref('saudi_hr_accommodation_book.mt_accommodation_refuse')
    ...
    check_in_time = fields.Datetime(string='Trip Start Date', required=True)
    check_out_time = fields.Datetime(string='Trip End Date', required=True)
    ...
    expense_total = fields.Float(compute=_calculate_expense_total, string='Total Expense', digits='Account')
    ...
    def name_get(self):
        ...
        for accommodation in self:
            ...
            name_list.extend([' - (', accommodation.check_in_time.strftime('%Y-%m-%d')])
            name_list.extend([' to ', accommodation.check_out_time.strftime('%Y-%m-%d'), ')'])
            name = ''.join(name_list)
    ...
    def confirm_accommodation(self):
        """
            sent the status of generating accommodation record in confirmed state
            :return: True
        """
        self.ensure_one()
        self.state = 'confirm'
    ...
    def accommodation_booked(self):
        ...
        self.message_post(message_type="email", subtype='mail.mt_comment', body=_("Hello, %s <br/>Your Accommodation Request is booked.<br/>Check In Date: %s <br/>Check Out Date: %s <br/>Check Further Details In Accommodation Request.") % (self.employee_id.name,self.check_in_time,self.check_out_time))
        self.state = 'booked'
    ...
    def generate_expense(self):
        ...
        """
        self.ensure_one()
        product_id = self.env.ref('saudi_hr_accommodation_book.hotel_accommodation')
...
class AccommodationLine(models.Model):
    ...
    accommodation_type = fields.Selection([('villa', 'Villa'), ('hotel', 'Hotel')], string='Stay In', required=True, track_visibility='onchange',
                                           default='villa')
    ...
    def name_get(self):
        ...
        for line in self:
            ...
            name_list.extend([', ', line.accommodation_type or ''])
            name_list.extend([' - (', line.check_in_time.strftime('%Y-%m-%d')])
            name_list.extend([' to ', line.check_out_time.strftime('%Y-%m-%d'), ')'])
            name = ''.join(name_list)
            res.append((line.id, name))
        return res

    @api.onchange('accommodation_type', 'villa_id', 'room_id', 'days')
    def onchange_stay(self):
        """
            onchange the value based on accommodation_type, villa, room, days
            amount
        """
        if not self.accommodation_type or not self.days:
            self.amount = 0.0
        stay_record = False
        if self.accommodation_type == 'villa' and self.villa_id:
            stay_record = self.villa_id
        elif self.accommodation_type == 'hotel' and self.room_id:
    ...
    def onchange_time_details(self):
        ...
        if (self.check_in_time and self.check_out_time) and (self.check_in_time <= self.check_out_time):
            diff_days = relativedelta(self.check_out_time, self.check_in_time)
            days = diff_days.days
            if diff_days.hours:
                days += 1
            elif diff_days.hours == 0 and diff_days.minutes >= 1:
                days += 1
            self.days = days
        else:
            warning.update({
