...
class HrPayslip(models.Model):
        ...
        for contract in contract_ids:
            ...
            if other_ids:
                ...
                for code, amount in input_allowance_lines.items():
                    input_type_id=False
                    if code == 'OTHER_ALLOWANCE_AMOUNT':
                        input_type_id=self.env.ref(
                            'saudi_hr_payroll.Other_Allowance').id
                    elif code == 'OTHER_ALLOWANCE_DAYS':
                       input_type_id= self.env.ref('saudi_hr_payroll.OTHER_ALLOWANCE_DAYS').id
                    elif code == 'OTHER_ALLOWANCE_HOURS':
                        input_type_id=self.env.ref('saudi_hr_payroll.OTHER_ALLOWANCE_HOURS').id
                    elif code == 'OTHER_ALLOWANCE_PERCENTAGE':
                        input_type_id=self.env.ref('saudi_hr_payroll.OTHER_ALLOWANCE_PERCENTAGE').id
                ...
                for code, amount in input_deduction_lines.items():
                    input_type_id=False
                    if code == 'OTHER_DEDUCTION_AMOUNT':
                        input_type_id=self.env.ref('saudi_hr_payroll.Other_Deduction').id
                    elif code == 'OTHER_DEDUCTION_DAYS':
                       input_type_id= self.env.ref('saudi_hr_payroll.OTHER_DEDUCTION_DAYS').id
                    elif code == 'OTHER_DEDUCTION_HOURS':
                        input_type_id=self.env.ref('saudi_hr_payroll.OTHER_DEDUCTION_HOURS').id
                    elif code == 'OTHER_DEDUCTION_PERCENTAGE':
                        input_type_id=self.env.ref('saudi_hr_payroll.OTHER_DEDUCTION_PERCENTAGE').id
    ...
    def _onchange_employee(self):
        ...
        self.input_line_ids =self.get_inputs(self.contract_id,self.date_from,self.date_to)
        return res
