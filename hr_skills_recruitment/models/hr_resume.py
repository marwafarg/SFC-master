...
class HrRecruitmentResumeLine(models.Model):
    ...
    def action_send_mail(self):
        ...
        """
        template_id = self.env.ref('hr_skills_recruitment.recruitment_email_template_employment_reference', False)
    ...
    def check_certificate_expiry(self):
        try:
            template_id = self.env.ref('hr_skills_recruitment.cerificate_expiry_notification')
        except ValueError:
            template_id = False
...
class HrApplicant(models.Model):
    ...
    resume_line_ids = fields.One2many('hr.recruitment.resume.line', 'applicant_id', string="Resumé lines")
    employee_skill_ids = fields.One2many('hr.recruitment.employee.skill', 'applicant_id', string="Skills")

    def case_close_with_emp(self):
        rec = super(HrApplicant, self).case_close_with_emp()
        if self.emp_id:
            for line in self.resume_line_ids:
                self.env['hr.resume.line'].create({'employee_id': self.emp_id.id or False,
                                                   'state': line.state,
