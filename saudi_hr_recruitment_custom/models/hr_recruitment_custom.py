...
class HrRecruitmentStage(models.Model):
    ...
    # TODO - Removed feedback field its not used anymore.
    ...
    # TODO - Removed Surveyed field its not used anymore.
...
class HRApplicant(models.Model):
    ...
    #     if applicant_id:
    #         try:
    #             template_id = self.env.ref('saudi_hr_recruitment_custom.email_template_applicant_acknowledge')
    #         except ValueError:
    #             template_id = False
    ...
    country_id = fields.Many2one('res.country', 'Nationality', required=True, default=_get_saudi_country)
    ...
    # interview_required = fields.Boolean(string="Interview_required", related='stage_id.interview_required', copy=False)
    ...
    def onchange_department_id(self):
        ...
        if self.department_id:
            job_req_ids = requisition_obj.search([('department_id', '=', self.department_id.id), ('state', '=', 'launch')])
    ...
    def case_close_with_emp(self):
        ...
        for applicant in self:
            ...
            if applicant.job_id and (applicant.partner_name or contact_name):
                job_req_obj = job_req_pool.search([('state', '=', 'launch'), ('job_id', '=', self.job_id.id)], limit=1)
                if not job_req_obj:
                    raise UserError(_("Kindly define Job Requisition for this job in Launch state."))
    ...
    def send_offer_letter(self):
        ...
        try:
            template_id = ir_model_data.get_object_reference('saudi_hr_recruitment_custom', 'email_template_applicant_offer')[1]
        except ValueError:
            template_id = False
        try:
...
class HRSurveyFeedback(models.Model):
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        res = super(HRSurveyFeedback, self).fields_view_get(view_id=view_id, view_type='form', toolbar=toolbar, submenu=submenu)
    ...
    def feedback_save(self):
        ...
        for record in self:
            ...
            if record.next_round_required and record.employee_id and is_view != 'refuse':
                try:
                    template_id = self.env.ref('saudi_hr_recruitment_custom.email_template_reminder_for_next_round')
                except ValueError:
                    template_id = False
                if template_id:
            ...
            if stage:
                ...
                self.write({'applicant_id': applicant_id,
